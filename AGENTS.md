...
- Front-ends: desktop "Graphs" toolbar action `graph_histogram_action` (`MainWindow::GraphHistogram`, icon `images/graphs/graph_histogram.png`) + context menu "Graph format"/"Copy image" find `GRAPH_HISTOGRAM`; web `#graph-histogram-button` → `OnGraphHistogram`, `IsGraph()`/`GetGraphFormat()`/`GetGraphImage()`/`OnGraphFormat` accept it. `PlotFormatDialog` takes a second `histogram` flag (desktop ctor `(PlotFormat&, surface, histogram)` reading/writing `histogram_style`; web EM_JS/vue pass `histogram`, the style value/combo index is the enum value directly - `OnPlotFormat` writes `histogram_style` for `GRAPH_HISTOGRAM` and `style` otherwise) and shows the bar style items for histograms, surface items for surfaces, nothing for line graphs. Desktop test `TestToolbar::testGraphHistogram` (run the whole `yutovo-desktop_test` — passing a single `TestToolbar::...` filter makes the first `qExec` of `TestFiles` exit the process); web `test/graph_histogram.spec.js` (`utils.insertGraphHistogram`). Rebuild order: yutovo-calculator wasm → yutovo-editor wasm (`build_web/debug` with `emcmake`) → yutovo-web, each installed into `$YUTOVO_DEPLOY` first.
